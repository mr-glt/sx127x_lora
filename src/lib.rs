...
impl<SPI, CS, RESET, DELAY, E> LoRa<SPI, CS, RESET, DELAY>
    ...
    pub fn new(spi: SPI, cs: CS, reset: RESET, frequency: i64, delay: DELAY)
        ...
        sx127x.reset.set_low();
        sx127x.delay.delay_ms(10);
        sx127x.reset.set_high();
        sx127x.delay.delay_ms(10);
        if sx127x.read_register(Register::RegVersion.addr()).unwrap() == 0x12 {
            sx127x.set_mode(RadioMode::Sleep);
            sx127x.set_frequency(frequency);
            sx127x.write_register(Register::RegFifoTxBaseAddr.addr(),0).unwrap();
            sx127x.write_register(Register::RegFifoRxBaseAddr.addr(),0).unwrap();
            let lna = sx127x.read_register(Register::RegLna.addr()).unwrap();
            sx127x.write_register(Register::RegLna.addr(), lna | 0x03).unwrap();
            sx127x.write_register(Register::RegModemConfig3.addr(), 0x04).unwrap();
    ...
    pub fn poll_irq(&mut self, timeout: Option<i32>) -> Result<usize,()>{
        ...
        match timeout {
            Some(value) => {
                ...
                if count >= (value * 10) {
                    ...
                    self.clear_irq();
                    Ok(self.read_register(Register::RegRxNbBytes.addr()).unwrap() as usize)
                }
            },
            None => {
                while !self.read_register(Register::RegIrqFlags.addr()).unwrap().get_bit(6) {
    ...
    pub fn read_packet(&mut self) -> [u8; 255] {
        let mut buffer = [0 as u8; 255];
        self.clear_irq();
        let size = self.read_register(Register::RegRxNbBytes.addr()).unwrap();
        let fifo_addr = self.read_register(Register::RegFifoRxCurrentAddr.addr()).unwrap();
        self.write_register(Register::RegFifoAddrPtr.addr(), fifo_addr).unwrap();
        for i in 0..size {
            let byte = self.read_register(Register::RegFifo.addr()).unwrap();
    ...
    pub fn transmitting(&mut self) -> bool {
        if (self.read_register(Register::RegOpMode.addr()).unwrap() & RadioMode::Tx.addr())
            ...
            if (self.read_register(Register::RegIrqFlags.addr()).unwrap()
                & IRQ::IrqTxDoneMask.addr()) == 1{
                self.write_register(Register::RegIrqFlags.addr(),
                                    IRQ::IrqTxDoneMask.addr()).unwrap();
            }
            false
        }
    }
    ...
    pub fn clear_irq(&mut self){
        let irq_flags = self.read_register(Register::RegIrqFlags.addr()).unwrap();
        self.write_register(Register::RegIrqFlags.addr(), irq_flags).unwrap();
    }
    ...
    /// Default value is `17`.
    pub fn set_tx_power(&mut self, mut level: i32, output_pin: u8){
        if PaConfig::PaOutputRfoPin.addr() == output_pin {
            ...
            self.write_register(Register::RegPaConfig.addr(), (0x70 | level) as u8).unwrap();
        } else {
            // PA BOOST
            if level > 17 {
                if level > 20 {
                ...
                self.write_register(Register::RegPaDac.addr(), 0x87).unwrap();
                self.set_ocp(140);
            }else {
                if level < 2 {
                    level = 2;
    ...
    pub fn set_ocp(&mut self, ma: u8){
        ...
        self.write_register(Register::RegOcp.addr(), 0x20 | (0x1F & ocp_trim)).unwrap();
    }
    ...
    pub fn set_frequency(&mut self, freq: i64){
        self.frequency = freq;
    ...
    fn set_explicit_header_mode(&mut self) {
        let reg_modem_config_1 = self.read_register(Register::RegModemConfig1.addr()).unwrap();
        self.write_register(Register::RegModemConfig1.addr(),reg_modem_config_1  & 0xfe).unwrap();
    ...
    fn set_implicit_header_mode(&mut self) {
        let reg_modem_config_1 = self.read_register(Register::RegModemConfig1.addr()).unwrap();
        self.write_register(Register::RegModemConfig1.addr(),reg_modem_config_1  & 0x01).unwrap();
    ...
    pub fn set_spreading_factor(&mut self, mut sf: u8) {
        if sf < 6 {
            sf = 6;
        } else if sf > 12 {
            sf = 12;
        ...
        if sf == 6 {
            self.write_register(Register::RegDetectionOptimize.addr(), 0xc5).unwrap();
            self.write_register(Register::RegDetectionThreshold.addr(), 0x0c).unwrap();
        } else {
            self.write_register(Register::RegDetectionOptimize.addr(), 0xc3).unwrap();
            self.write_register(Register::RegDetectionThreshold.addr(), 0x0a).unwrap();
        }
        let modem_config_2 = self.read_register(Register::RegModemConfig2.addr()).unwrap();
    ...
    /// Default value is `5`.
    pub fn set_coding_rate_4(&mut self, mut denominator: u8) {
        if denominator < 5 {
            denominator = 5;
    ...
    pub fn set_crc(&mut self, value: bool){
        let modem_config_2 = self.read_register(Register::RegModemConfig2.addr()).unwrap();
        if value {
            self.write_register(Register::RegModemConfig2.addr(), modem_config_2 | 0x04).unwrap();
        }else{
            self.write_register(Register::RegModemConfig2.addr(), modem_config_2 & 0xfb).unwrap();
        }
    }

    /// Inverts the radio's IQ signals. Default value is `false`.
    pub fn set_invert_iq(&mut self, value: bool) {
        if value {
            self.write_register(Register::RegInvertiq.addr(),  0x66).unwrap();
            self.write_register(Register::RegInvertiq2.addr(), 0x19).unwrap();
        }else{
            self.write_register(Register::RegInvertiq.addr(),  0x27).unwrap();
            self.write_register(Register::RegInvertiq2.addr(), 0x1d).unwrap();
        }
    }


    /// Returns the spreading factor of the radio.
    pub fn get_spreading_factor(&mut self) -> u8{
        self.read_register(Register::RegModemConfig2.addr()).unwrap() >> 4
    }
    ...
    fn set_ldo_flag(&mut self) {
        ...
        let mut config_3 = self.read_register(Register::RegModemConfig3.addr()).unwrap();
        config_3.set_bit(3,ldo_on);
        self.write_register(Register::RegModemConfig3.addr(), config_3).unwrap();
    }
    ...
    fn read_register(&mut self, reg: u8) -> Result<u8,()> {
        ...
        let transfer = self.spi.transfer(&mut buffer);
        self.cs.set_high();
        match transfer {
            Ok(buffer) => Ok(buffer[1]),
            Err(_e) => Err(()),
        }
    }
    ...
    fn write_register(&mut self, reg: u8, byte: u8) -> Result<(),()>{
        ...
        let write = self.spi.write(& buffer);
        self.cs.set_high();
        match write {
            Ok(()) => Ok(()),
            Err(_e) => Err(()),
